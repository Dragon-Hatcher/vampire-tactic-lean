...
their literals: theory axioms, and ALASCA's inferences that add inequalities
together. The literal-wise simplifications are `Vampire.LiteralRewrite`'s.

Kept apart from the replay because the replay is precompiled and imports no
Mathlib, while these steps are proved with Mathlib's `linarith`, `omega` and
lemmas. Replay is handed `contradiction` and `cancelling` rather than
importing them.
-/
...
/-- `goal` by `linarith` from `facts`, preprocessing them as `cfg` says. -/
private def byLinarithWith (facts : List Expr) (goal : Expr)
...
private def askAbout (facts : Array Expr) (claim : Option Expr) : MetaM Expr := do
  ...
  let byLinarith : MetaM Expr :=
    byLinarithWith facts.toList (claim.getD (mkConst ``False))
...
def contradiction (facts : Array Expr) (claim : Option Expr) : MetaM Expr := do
  ...
  | some c => do
    if let some fact ← stating facts c then return fact
    VampireReplay.Abstract.abstracting askAbout facts claim
where
  /-- A fact that states the claim: then there is nothing to ask, and over a
  sort that is not numbers no procedure could be asked. -/
